window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function formatScheduleFrequency(record, t) {
			if (record.kind === "daily") return t("frequency.daily");
			if (record.kind === "weekly") return t("frequency.weekly");
			if (record.kind === "cron") return t("frequency.cron");
		...
		function flattenCatalogRows(records, snapshot) {
			const byId = snapshot?.byId ?? {};
			const out = [];
			for (const record of Array.isArray(records) ? records : []) {
				...
				const session = byId[sessionId] ?? { id: sessionId, displayTitle: sessionId };
		...
		function projectionReadState(snapshot, canRefresh) {
			...
			for (const id of ids) {
				...
				const legacy = byId[id]?.projectionValues;
		...
		function SchedulePanel({ t }) {
			...
			const remote = hostCtx?.remote;
			const hasHostCatalog = typeof remote?.schedule?.catalog === "function";
			const [snapshot, setSnapshot] = react.useState(() => sessions?.list?.getSnapshot());
			const [catalog, setCatalog] = react.useState(() => ({
				records: [],
				status: hasHostCatalog ? "loading" : "unavailable"
			}));
			react.useEffect(() => {
			...
			react.useEffect(() => {
				if (!hasHostCatalog) {
					setCatalog({ records: [], status: "unavailable" });
					return;
				}
				let alive = true;
				const refresh = async () => {
					...
					try {
						...
						setCatalog(result?.ok
							? { records: Array.isArray(result.value) ? result.value : [], status: "ready" }
							: { records: [], status: "error" });
					} catch {
						if (alive) setCatalog({ records: [], status: "error" });
					}
				};
				const stopChanged = typeof remote?.$on === "function"
					? remote.$on("schedule/changed", () => { void refresh(); })
					: undefined;
				const stopReset = typeof hostCtx?.on === "function"
					? hostCtx.on("connection/reset", () => { void refresh(); })
					: undefined;
				void refresh();
				return () => {
					alive = false;
					if (typeof stopChanged === "function") stopChanged();
					if (typeof stopReset === "function") stopReset();
				};
			}, [hasHostCatalog, remote]);
			const canRefreshProjections = typeof sessions?.refreshProjections === "function";
			react.useEffect(() => {
				if (hasHostCatalog || !canRefreshProjections) return;
				const ids = snapshot?.ids ?? [];
				const projections = snapshot?.projectionsBySession ?? {};
				for (const id of ids) {
			...
			}, [snapshot, canRefreshProjections, hasHostCatalog]);
			const flat = hasHostCatalog
				? flattenCatalogRows(catalog.records, snapshot)
				: flattenScheduleRows(snapshot);
			const projectionState = hasHostCatalog
				? catalog.status
				: projectionReadState(snapshot, canRefreshProjections);
								...
								? react_jsx_runtime.jsxs("div", {
									...
									children: [
										...
										react_jsx_runtime.jsx("div", {
											...
											children: t(projectionState === "loading"
												? "empty.loading.title"
